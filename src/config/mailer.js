...
const sendMailToRegister = async (userMail, token) => {
  try {
    ...
    email.sender = { name: "AmiKuna", email: process.env.EMAIL_FROM };
    email.to = [{ email: userMail }];
    email.subject = "❤️🔥 AmiKuna 🔥❤️ - Confirma tu cuenta para empezar";
    email.htmlContent = `
      <!DOCTYPE html>
      <html lang="es">
...
const sendMailToRecoveryPassword = async (userMail, token) => {
  try {
    ...
    email.sender = { name: "AmiKuna", email: process.env.EMAIL_FROM };
    email.to = [{ email: userMail }];
    email.subject = "🔐 AmiKuna - Recupera tu contraseña";
    email.htmlContent = `
